;; NOTE: The original author of this file was Jordan Schaefer. It was modified
;; by me, Jack Crawley. The original is part of Elpy.
;;
;; https://github.com/jorgenschaefer/elpy


(defun elpy2-snippet-split-args (arg-string)
  "Split a python argument string into ((name, default)..) tuples"
  (mapcar (lambda (x)
            (split-string x "[[:blank:]]*=[[:blank:]]*" t))
          (split-string arg-string "[[:blank:]]*,[[:blank:]]*" t)))

(defun elpy2-snippet-current-method-and-args ()
...
(defun elpy2-snippet-init-assignments (arg-string)
    ...
    (mapconcat (lambda (arg)
                 (if (string-match "^\\*" (car arg))
                   ...
                   (format "self.%s = %s\n%s"
                           ...
                           (car arg)
                           indentation)))
               (elpy2-snippet-split-args arg-string)
               "")))

(defun elpy2-snippet-super-form ()
  "Return (Class, first-arg).method if Py2.
Else return ().method for Py3."
  (let* ((defun-info (elpy2-snippet-current-method-and-args))
         ...
         (py-version-num (substring (shell-command-to-string (concat elpy2-rpc-python-command py-version-command))0 1)))
    (if (string-match py-version-num "2")
        (format "(%s, %s).%s" class first-arg method)
      (format "().%s" method))))

(defun elpy2-snippet-super-arguments ()
  "Return the argument list for the current method."
  (mapconcat (lambda (x) x)
             (cdr (nth 2 (elpy2-snippet-current-method-and-args)))
             ", "))
